...
void VictronSmartShuntComponent::dump_config() {
  ...
  LOG_SENSOR("  ", "Minimum aux (battery) voltage", min_aux_battery_voltage_sensor_);
  LOG_SENSOR("  ", "Maximum aux (battery) voltage", max_aux_battery_voltage_sensor_);
  ...
  LOG_SENSOR("  ", "Cumulative Amp Hours Drawn", cumulative_amp_hours_drawn_sensor_);
  LOG_SENSOR("  ", "Number of Automatic Synchronizations", number_of_automatic_synchronizations_sensor_);
  LOG_SENSOR("  ", "Number of Low Main Voltage Alarms", number_of_low_main_voltage_alarms_sensor_);
  LOG_SENSOR("  ", "Number of High Main Voltage Alarms", number_of_high_main_voltage_alarms_sensor_);
...
static const std::string alarm_reason_text(int value) {
  switch (value) {
    case 1:
      return "Low Voltage";
    case 2:
...
void VictronSmartShuntComponent::handle_value_() {
  ...
  if (label_ == "AR") {
    ...
    if (alarm_reason_text_sensor_ != nullptr)
      alarm_reason_text_sensor_->publish_state(alarm_reason_text(value));  // NOLINT(cert-err34-c)
  } else if (label_ == "H5") {
    ...
    if (depth_of_average_discharge_sensor_ != nullptr)
      // mAh -> Ah
      depth_of_average_discharge_sensor_->publish_state(atoi(value_.c_str()) / 1000.0);  // NOLINT(cert-err34-c)
  } else if (label_ == "H6") {
    ...
    if (min_aux_battery_voltage_sensor_ != nullptr)
      // mV to V
      min_aux_battery_voltage_sensor_->publish_state(atoi(value_.c_str()) / 1000.00);  // NOLINT(cert-err34-c)
  } else if (label_ == "H16") {
    if (max_aux_battery_voltage_sensor_ != nullptr)
      // mV to V
