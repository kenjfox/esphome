...
CONF_MIN_AUX_BATTERY_VOLTAGE = "min_aux_battery_voltage"
CONF_MAX_AUX_BATTERY_VOLTAGE = "max_aux_battery_voltage"
...
CONF_DEPTH_OF_AVERAGE_DISCHARGE = "depth_of_average_discharge"
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_MIN_AUX_BATTERY_VOLTAGE): sensor.sensor_schema(
            unit_of_measurement=UNIT_VOLT,
            icon=ICON_FLASH,
            accuracy_decimals=3,
            device_class=DEVICE_CLASS_VOLTAGE,
        ),
        cv.Optional(CONF_MAX_AUX_BATTERY_VOLTAGE): sensor.sensor_schema(
            unit_of_measurement=UNIT_VOLT,
            icon=ICON_FLASH,
            accuracy_decimals=3,
            device_class=DEVICE_CLASS_VOLTAGE,
        ...
        cv.Optional(CONF_ALARM_REASON): sensor.sensor_schema(
            unit_of_measurement=UNIT_EMPTY,
            icon=ICON_EMPTY,
            accuracy_decimals=0,
            device_class=DEVICE_CLASS_EMPTY,
        ...
        cv.Optional(CONF_DEPTH_OF_AVERAGE_DISCHARGE): sensor.sensor_schema(
            unit_of_measurement=UNIT_AMPERE,
            icon=ICON_CURRENT_AC,
            accuracy_decimals=3,
            device_class=DEVICE_CLASS_POWER,
...
def to_code(config):
    ...
    if CONF_ALARM_REASON in config:
        sens = yield sensor.new_sensor(config[CONF_ALARM_REASON])
        cg.add(var.set_alarm_reason_sensor(sens))

    if CONF_ALARM_REASON_TEXT in config:
        conf = config[CONF_ALARM_REASON_TEXT]
        sens = cg.new_Pvariable(conf[CONF_ID])
        yield text_sensor.register_text_sensor(sens, conf)
        cg.add(var.set_alarm_reason_text_sensor(sens))

    if CONF_DEPTH_OF_AVERAGE_DISCHARGE in config:
        sens = yield sensor.new_sensor(config[CONF_DEPTH_OF_AVERAGE_DISCHARGE])
        cg.add(var.set_depth_of_average_discharge_sensor(sens))

    if CONF_CUMULATIVE_AMP_HOURS_DRAWN in config:
        sens = yield sensor.new_sensor(config[CONF_CUMULATIVE_AMP_HOURS_DRAWN])
        cg.add(var.set_cumulative_amp_hours_drawn_sensor(sens))

    if CONF_NUMBER_OF_AUTOMATIC_SYNCHRONIZATIONS in config:
        sens = yield sensor.new_sensor(
            config[CONF_NUMBER_OF_AUTOMATIC_SYNCHRONIZATIONS]
        )
        cg.add(var.set_number_of_automatic_synchronizations_sensor(sens))

    if CONF_NUMBER_OF_LOW_MAIN_VOLTAGE_ALARMS in config:
        sens = yield sensor.new_sensor(config[CONF_NUMBER_OF_LOW_MAIN_VOLTAGE_ALARMS])
        cg.add(var.set_number_of_low_main_voltage_alarms_sensor(sens))

    if CONF_NUMBER_OF_HIGH_MAIN_VOLTAGE_ALARMS in config:
        sens = yield sensor.new_sensor(config[CONF_NUMBER_OF_HIGH_MAIN_VOLTAGE_ALARMS])
        cg.add(var.set_number_of_high_main_voltage_alarms_sensor(sens))
